...
def travel_eticket(request, pk):
    reservation = get_object_or_404(Reservation, pk=pk)

    # Render the HTML template with the booking details
    html_string = render_to_string('pages/UserDashboard/traveleticket.html', {'reservation': reservation})

    # Create an HttpResponse object and set the content type to application/pdf
    response = HttpResponse(content_type='application/pdf')

    # Set the file name for the downloaded PDF
    response['Content-Disposition'] = f'attachment; filename="e-ticket-{reservation.id}.pdf"'
